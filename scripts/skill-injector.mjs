...
var DEFAULT_DFH_AUTOPILOT_CONFIG = {
  maxIterations: 12,
  pipeline: {
...
function buildPipelineStages(config) {
  return [
    {
      id: "analyst",
      phase: "expansion",
...
function clearAutopilotState(directory) {
  const paths = resolveAutopilotPaths(directory);
  if (!existsSync(paths.stateFile)) {
    return true;
  }
...
function initAutopilot(directory, request, sessionId, config) {
  const mergedPipelineConfig = {
...
// src/hooks/skill-injector/index.ts
function isAutopilotKeyword(prompt) {
  const primary = getPrimaryKeyword(prompt)?.type;
  return primary === "autopilot" || primary === "dfh-autopilot";
}
function isCancelKeyword(prompt) {
  return getPrimaryKeyword(prompt)?.type === "cancel";
}
function stripLeadingAutopilotKeyword(prompt) {
  return prompt.replace(/\b(dfh-autopilot|dfh autopilot|autopilot|auto pilot|auto-pilot)\b\s*:?\s*/i, "").trim();
}
...
function processSkillInjection(input) {
  const prompt = extractPrompt(input);
  ...
  if (isCancelKeyword(prompt)) {
    const existing2 = readAutopilotState(directory);
    if (!existing2?.active) {
      return createSuppressOutput();
    }
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith("skill-injector.mjs")) {
  void main2();
}
export {
  main2 as main,
...
//# sourceMappingURL=skill-injector.mjs.map
