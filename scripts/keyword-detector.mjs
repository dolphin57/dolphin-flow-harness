...
function resolveConflicts(matches) {
  ...
  const priorityOrder = ['cancel','ralph','autopilot','team','ultrawork',
    'pipeline','ccg','ralplan','plan','tdd','research','ultrathink','deepsearch','analyze','dfh-refactor'];
...
async function main() {
  ...
  try {
    ...
    // DFH Refactor keyword (Dolphin Flow Harness)
    if (/\b(dfh-refactor|dfh refactor)\b/i.test(cleanPrompt)) {
      matches.push({ name: 'dfh-refactor', args: '' });
    }
