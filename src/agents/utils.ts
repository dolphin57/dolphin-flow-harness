/**
 * Agent Utilities for Dolphin Flow Harness
 *
...
export function parseDisallowedTools(agentName: string): string[] | undefined {
  ...
  try {
    ...
    return disallowedMatch[1].split(',').map((t: string) => t.trim()).filter(Boolean);
  } catch {
    return undefined;
  }
}

export const OPEN_QUESTIONS_PATH = '.dfh/plans/open-questions.md';
